...
def test_collect_inter_human_skips_completed_repos_from_checkpoint(
    tmp_path, monkeypatch
):
    """Repos already persisted via checkpoint must be skipped on re-run."""
    clones_dir = tmp_path / "clones"
    clones_dir.mkdir()
    out_db = tmp_path / "between.db"
    ...
    repos = [
        {
            "full_name": "owner/already-done",
            "language": "python",
            "clone_url": "https://example.com/owner/already-done.git",
            "github_id": 1,
        },
        {
            "full_name": "owner/still-pending",
            "language": "python",
            "clone_url": "https://example.com/owner/still-pending.git",
            "github_id": 2,
        },
    ]
    ...
    class FakeExtractor:
        ...
        def _extract_from_agent_commits(self, repo_name, commits):
            extracted_repos.append(repo_name)
            return [
                {
                    "name": "my_fixture",
                    "file_path": "tests/test_foo.py",
    ...
    # Pre-seed a checkpoint claiming 'owner/already-done' is already persisted.
    from collection.human_corpus import _save_inter_checkpoint

    _save_inter_checkpoint(
        tmp_path / "human_inter_checkpoint.json",
        {"owner/already-done"},
        {"repos_persisted": 1, "fixtures_persisted": 1},
    )
    ...
    stats, db_path = collector.collect_inter_human(
        repos, targets={"python": 1}, workers=1
    )
