...
def test_persist_and_insert_inter_monkeypatched(tmp_path, monkeypatch):
    ...
    collector = HumanCorpusCollector(corpus_db_path=out_db, clones_dir=tmp_path / "clones", output_db=out_db)
    ...
    def fake_persist(output_db, repo_data, fixtures_list, out_path=None, handle_mocks=False):
    ...
    monkeypatch.setattr(db_module, 'insert_human_inter_fixtures_coordinated', lambda db_path, sel, seed=42, batch_size=1000: len(sel))

    inter_checkpoint = tmp_path / 'human_inter_checkpoint.json'
    inter_progress = tmp_path / 'between_human_inter_progress.json'

    counts_local, completed_repos, inserted = collector._persist_and_insert_inter(selected, inter_checkpoint, inter_progress, seed=42)
