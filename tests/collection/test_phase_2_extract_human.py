...
import collection.phase_2_extract_human as phase2
...
def test_phase_2_errors_when_clones_dir_missing(monkeypatch, tmp_path):
    source_db = tmp_path / "source.db"
    source_db.write_text("stub", encoding="utf-8")
    missing_clones_dir = tmp_path / "no-such-clones"
    output_db = tmp_path / "human.db"

    monkeypatch.setattr(
        phase2, "HumanCorpusCollector", lambda *a, **k: DummyHumanCollector(*a, **k)
    )
    monkeypatch.setattr(
        phase2.sys,
        "argv",
