...
- [`collection/heuristics/agent_files.csv`](../../collection/heuristics/agent_files.csv) — config-file/directory patterns (`file_based`), as a flat `pattern,tool,start_date,end_date` table. This deliberately mirrors [labri-progress/agent-mining](https://github.com/labri-progress/agent-mining)'s own `files.csv` schema and content: the file's first 95 data rows are that file's content verbatim in its original order, with this project's own additions appended after a `#`-prefixed comment line marking the boundary. The 11 additions are patterns this project already matched before this migration that upstream's list doesn't have (e.g. bare `.devin`/`.openhands`/`.cline`, `claude.config`/`cursor.config`). Entries ending in `/` match a directory name anywhere in the path; others may use fnmatch globs (see `collection/agent_patterns.py:path_matches_pattern`).
