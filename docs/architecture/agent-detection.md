...
- [`collection/heuristics/agent_heuristics.yaml`](../../collection/heuristics/agent_heuristics.yaml) holds only the paper's strict-scope subset (`paper_scope`) now; `file_based`/`commit_signatures`/`bot_patterns` all moved out to their own CSVs, below.
