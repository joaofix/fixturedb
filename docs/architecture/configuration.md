...
| Temporal boundaries, quality thresholds, sampling parameters | `collection/study_parameters/study_parameters.yaml` | `config.HUMAN_CORPUS_CUTOFF_DATE` / `MIN_STARS` / `MIN_COMMITS` / `MIN_TEST_FILES` / `MIN_NON_BLANK_LOC` / `MIN_FIXTURES_FOUND` / `TARGET_REPOS_PER_LANGUAGE_BETWEEN_GROUP` / `DATASET_C_SAMPLING_SEED` |
...
| Mock-framework, external-call regex tables + setup/teardown pairing rules | `collection/heuristics/feature_extraction_patterns.yaml` | `detector_shared.py` (`MOCK_PATTERNS`, `EXTERNAL_CALL_PATTERNS`, teardown pairing) |
...
The feature-extraction-patterns catalog covers the metrics computed *after* a fixture is already detected — `num_mocks`, `num_external_calls`, `has_teardown_pair` — namely what regex signals a mock/I-O-call, and which setup fixture types pair with which teardown fixture types. The teardown-pairing table only lists fixture types this codebase's detectors actually produce (TestNG, Mocha, AVA, `before_all`/`after_all`, JUnit3, JUnit4/pytest, ...); .NET-only types like `nunit_setup`/`xunit_fact`/`xunit_theory` are out of scope and not included. `num_objects_instantiated` used to be here too (a regex table) but isn't anymore — it's an AST node-type walk in `detector_shared.py` now, not a pattern-table lookup, see [Metrics Reference § num_objects_instantiated](metrics-reference.md#num_objects_instantiated) for why.
