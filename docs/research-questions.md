...
| `rq2.py` | RQ2 — per-dataset `fixture_type` kind (setup/teardown/other) distribution, plus an A vs C comparison of per-repo setup_pct/teardown_pct medians and a repo-level effect size/p-value per language | `db/a.db`, `db/c.db` | `research_questions/rq2.md` |
