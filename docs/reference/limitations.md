...
### Agent-Identity Name Collisions (`devin`/`cline`)
- **Problem:** manual validation review of Dataset A's `agent-commits-dataset-a` sample (2026-07-17, live-verified against real GitHub commits) found real humans misattributed to the Devin/Cline agents via exact name/email/employer-domain collisions with no trailer to disambiguate — e.g. authors literally named "Devin Jameson"/"Devin Robison", a human surnamed "Cline", and actual Cline-company employees (including its creator) committing to their own product's repo under an `@cline.bot` work email. See [Agent Detection § Known Limitations](../architecture/agent-detection.md#known-limitations) for the full investigation and fix (the root patterns causing the collision were removed from `agent_authors.csv`, not case-by-case allowlisted).
- **Measured impact on already-collected data:** re-running the fixed catalog's author-identity check against the existing `datasets/a/commits/*.csv` finds 3,467 of 10,228 `devin`-tagged and all 2,102 `cline`-tagged commits no longer match (upper-bound estimate — the commit message/trailer isn't persisted in the CSV, so a fraction may have genuinely matched via trailer instead). 357 already-extracted fixtures in `datasets/a/fixtures/*.csv` are attributed to this collision.
- **Mitigation (forward-looking, not retroactive):** the collision-causing patterns are removed from `collection/heuristics/agent-mining/agent_authors.csv`, effective for the next collection run. The real Devin AI bot identity (`devin-ai-integration`) is untouched and still correctly detected.
- **Impact:** the existing `datasets/a/commits/*.csv`, `datasets/a/fixtures/*.csv`, and `db/a.db` are not retroactively patched — Dataset A will be recollected later under the corrected catalog. Analysis drawing on the current corpus should account for this known contamination until that recollection happens.
