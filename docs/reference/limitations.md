...
- **Problem:** manual validation review of Dataset A's `agent-commits-dataset-a` sample (2026-07-17, live-verified against real GitHub commits) found real authors misattributed via exact name/email/employer-domain collisions with no trailer to disambiguate — e.g. authors literally named "Devin Jameson"/"Devin Robison", a human surnamed "Cline", actual Cline-company employees (including its creator) committing under an `@cline.bot` work email, and a repo-internal placeholder bot (`codex-review@example.com`) unrelated to real OpenAI Codex. See [Agent Detection § Known Limitations](../architecture/agent-detection.md#known-limitations) for the full investigation and both fixes (`devin`/`cline`'s root patterns removed from `agent_authors.csv` entirely; `codex-review@example.com` added to `known_human_collisions.csv`, since the bare `codex` pattern itself is still needed for genuine trailer-based detection elsewhere).
- **Measured impact on already-collected data:** re-running the fixed catalog's author-identity check against the existing `datasets/a/commits/*.csv` finds 3,467 of 10,228 `devin`-tagged and all 2,102 `cline`-tagged commits no longer match (upper-bound estimate — the commit message/trailer isn't persisted in the CSV, so a fraction may have genuinely matched via trailer instead), plus 226 `codex`-tagged commits from the placeholder identity. 357 + 204 already-extracted fixtures in `datasets/a/fixtures/*.csv` are attributed to these collisions.
- **Mitigation (forward-looking, not retroactive):** the collision-causing `devin`/`cline` patterns are removed from `collection/heuristics/agent-mining/agent_authors.csv`; the `codex` placeholder identity is excluded via `known_human_collisions.csv`. Both effective for the next collection run. The real Devin AI bot identity (`devin-ai-integration`) and genuine trailer-based Codex detection are untouched and still correctly detected.
