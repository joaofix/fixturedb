...
This script samples fixtures from both fixturedb-human.db and fixturedb-agent.db
...
Input:
  ...
  - fixturedb-human.db (from Phase 2)
    - fixturedb-agent.db (from Phase 3)
...
def main():
    ...
    agent_fixtures = load_fixtures_from_db(agent_db)

    logger.info(f"Human fixtures available: {len(human_fixtures)}")
    logger.info(f"Agent fixtures available: {len(agent_fixtures)}")

    if not human_fixtures or not agent_fixtures:
    ...
    try:
        ...
        logger.info("")
        logger.info("Sampling from agent dataset...")
        agent_result = sampler.sample(
            agent_fixtures,
            target_count=target_count,
            stratify_by="fixture_type",
            tolerance=0.02,
        ...
        logger.info(
            f"  All strata within tolerance: {human_stats['all_strata_within_tolerance']}"
        )
        logger.info("")
        ...
        logger.info("")
        logger.info("Agent Dataset Sampling:")
        logger.info(f"  Target: {agent_result.target_count}")
        logger.info(f"  Sampled: {agent_result.sampled_count}")
        logger.info(f"  Stratify by: {agent_result.stratify_by}")

        agent_stats = sampler.get_sample_statistics(agent_result)
        logger.info(
            f"  All strata within tolerance: {agent_stats['all_strata_within_tolerance']}"
        )
        logger.info("")
        logger.info("  Distribution check (fixture_type):")
        ...
        output_data = {
            ...
            "human_sampling": {
                ...
                "all_strata_within_tolerance": human_stats[
                    "all_strata_within_tolerance"
                ],
                "distribution_check": human_result.distribution_check,
                "sampled_fixture_ids": human_result.sampled_ids,
            },
            "agent_sampling": {
                "sampled_count": agent_result.sampled_count,
                "target_count": agent_result.target_count,
                "all_strata_within_tolerance": agent_stats[
                    "all_strata_within_tolerance"
                ],
                "distribution_check": agent_result.distribution_check,
                "sampled_fixture_ids": agent_result.sampled_ids,
            },
        }
