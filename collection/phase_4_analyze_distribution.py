...
strategy for balancing human vs LLM datasets.
...
def main():
    ...
    if not llm_db.exists():
        logger.error(f"fixturedb-llm.db not found: {llm_db}")
    ...
    try:
        ...
        logger.info("")
        logger.info("Analyzing fixturedb-llm.db...")
        llm_stats = analyze_database_distribution(llm_db)
        logger.info(f"  Total fixtures: {llm_stats['total_fixtures']}")
        logger.info(f"  Repositories: {llm_stats['repositories']}")
        logger.info(f"  Test files: {llm_stats['test_files']}")
        ...
        for fixture_type, count in sorted(
            human_stats['by_type'].items(),
            key=lambda x: x[1],
            reverse=True
        ):
            pct = (count / human_stats['total_fixtures'] * 100) if human_stats['total_fixtures'] else 0
            logger.info(f"  {fixture_type}: {count} ({pct:.1f}%)")

        logger.info("")
        logger.info("LLM Fixtures by Type:")
        for fixture_type, count in sorted(
            llm_stats['by_type'].items(),
            key=lambda x: x[1],
            reverse=True
        ):
            pct = (count / llm_stats['total_fixtures'] * 100) if llm_stats['total_fixtures'] else 0
            logger.info(f"  {fixture_type}: {count} ({pct:.1f}%)")

        logger.info("")
        logger.info("Human Fixtures by Scope:")
        for scope, count in sorted(
            human_stats['by_scope'].items(),
            key=lambda x: x[1],
            reverse=True
        ):
            pct = (count / human_stats['total_fixtures'] * 100) if human_stats['total_fixtures'] else 0
        ...
        logger.info("")
        logger.info("LLM Fixtures by Scope:")
        for scope, count in sorted(
            llm_stats['by_scope'].items(),
            key=lambda x: x[1],
            reverse=True
        ):
            pct = (count / llm_stats['total_fixtures'] * 100) if llm_stats['total_fixtures'] else 0
        ...
        min_count = min(human_stats['total_fixtures'], llm_stats['total_fixtures'])
        max_count = max(human_stats['total_fixtures'], llm_stats['total_fixtures'])
        ...
        logger.info(f"LLM fixtures available: {llm_stats['total_fixtures']}")
