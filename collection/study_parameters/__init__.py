"""Loader for study-design reference data.
...
- study_parameters.yaml -- temporal boundaries, quality-filter thresholds,
  and sampling parameters for the between-group methodology (see that
  file's header comment)

collection/config.py loads all four and derives the module-level constants
(NON_CODE_EXTENSIONS, FRAMEWORK_REGISTRY, LANGUAGE_CONFIGS,
HUMAN_CORPUS_CUTOFF_DATE, MIN_STARS, etc.) existing call sites already use
-- editing a catalog is a YAML change, not a Python change.
"""
...
def load_study_parameters() -> Dict[str, Any]:
    """Return the parsed study-design parameter catalog (temporal
    boundaries, quality thresholds, sampling parameters) -- see
    study_parameters.yaml's header for the full list.
    """
    return _load_yaml("study_parameters.yaml")
