...
def main():
    ...
    try:
        ...
        for i, repo_meta in enumerate(corpus_repos, 1):
            ...
            if not repo_path.exists():
                logger.debug(f"[{i}/{len(corpus_repos)}] Repo not found on disk: {repo_name}")
                continue
        ...
        logger.info(f"Repositories with agent commits: {tier1_results['repos_with_agent_commits']}")
        logger.info(f"Total agent commits found: {tier1_results['total_agent_commits']}")
        logger.info(f"Agent distribution:")
        for agent, count in sorted(tier1_results['agents_by_type'].items(), key=lambda x: x[1], reverse=True):
        ...
        logger.info("")
        logger.info("3. If Tier 1 insufficient, run: python phase_1d_discover_matched_repos.py")
