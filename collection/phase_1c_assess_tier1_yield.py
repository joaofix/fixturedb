...
Output: Decision report and recommendation for Phase 1D.
"""
...
def main():
    ...
    logger.info(f"  Minimum repos for Tier 1: {min_repos} (found: {repos_with_agent}) {'✓' if repos_sufficient else '✗'}")
    logger.info(f"  Minimum commits for Tier 1: {min_commits} (found: {total_agent_commits}) {'✓' if commits_sufficient else '✗'}")
    logger.info("")
    ...
    # Save assessment
    assessment_file = output_dir / f'phase_1c_assessment_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    ...
    if tier1_sufficient:
        ...
        logger.info("")
        logger.info("The corpus repos provide adequate agent fixture data for statistical power.")
        logger.info("You can proceed directly to Phase 2 (extract pre-2021) and Phase 3 (extract LLM).")
        logger.info("")
        logger.info("Next steps:")
        logger.info("  1. python phase_2_extract_pre_2021.py (extract human fixtures)")
        logger.info("  2. python phase_3_extract_llm.py (extract LLM fixtures with tier=1)")
        ...
        logger.info("Tier 2 matching (supplementary repos) is recommended to reach statistical power.")
        logger.info("")
        logger.info(f"Gap: Need {min_repos - repos_with_agent} more repos or {min_commits - total_agent_commits} more commits")
        logger.info("")
        logger.info("Next steps:")
        logger.info("  1. python phase_1d_discover_matched_repos.py (find Tier 2 repos via SEART)")
        logger.info("  2. Integrate Tier 2 repos into collection pipeline")
        logger.info("  3. python phase_2_extract_pre_2021.py (extract human fixtures)")
        logger.info("  4. python phase_3_extract_llm.py (extract LLM fixtures with tier labels)")
