...
def _load_agent_test_commit_resume_state(
    output_dir: Path,
    ...
    if output_dir.exists():
        for csv_path in sorted(output_dir.glob("*_agent_test_commit_qc.csv"), key=lambda p: p.name):
            language = csv_path.name.replace("_agent_test_commit_qc.csv", "")
            with csv_path.open("r", encoding="utf-8", newline="") as fh:
...
def collect_human_test_commits(repo_qc_dir: Path, output_dir: Path, clones_dir: Path = CLONES_DIR, workers: int = 12) -> dict:
    ...
    if workers == 1:
        for repo_name, repo_rows in grouped.items():
            language, repo_test_commits, repo_count, repo_commits_scanned = _process_repo_human_test_commits(repo_rows[0])
            repos_processed += repo_count
            commits_scanned += repo_commits_scanned
            ...
            pct = (repos_processed / total_repos * 100) if total_repos else 100
            logger.info("[human-test-commits] Progress: %d/%d repos (%.1f%%), %d commits scanned", repos_processed, total_repos, pct, commits_scanned)
    else:
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(_process_repo_human_test_commits, repo_rows[0]): repo_name for repo_name, repo_rows in grouped.items()}
            try:
                for future in as_completed(futures):
                    language, repo_test_commits, repo_count, repo_commits_scanned = future.result()
                    repos_processed += repo_count
                    ...
                    if repo_test_commits:
                        ...
                        test_commit_rows_by_language[language].extend(repo_test_commits)
                    pct = (repos_processed / total_repos * 100) if total_repos else 100
                    logger.info("[human-test-commits] Progress: %d/%d repos completed (%.1f%%), %d commits scanned", repos_processed, total_repos, pct, commits_scanned)
            except KeyboardInterrupt:
...
def _cli_main() -> None:
    ...
    if args.mode == "agent":
        ...
        collect_human_test_commits(args.repo_qc_dir, args.output_dir, workers=args.workers)
