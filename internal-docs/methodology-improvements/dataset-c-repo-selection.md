...
## 10. LOC re-verified at cutoff too (2026-08-13)

Section 3's "no live GitHub metadata" fix covered stars (dropped) and commits (re-measured at cutoff), but missed a third dimension: [Dataset Card](../../docs/data/dataset-card.md) documents that `github-search-raw/` also pre-filters candidates at *source* to `≥5k LOC` — the exact same crawl-time-not-cutoff-time problem as the original commit-count issue in section 2, just unnoticed at the time (SEART's query criteria weren't the focus of that review). A repo could have been a tiny toy project back at its own pre-2021 snapshot and only grown past 5k lines well afterward, same as a repo that "has 100+ commits today."

Fixed the same way as the commit-count fix: `dataset_c.py::_process_repo()` now also computes real non-blank LOC from the repo's own checked-out cutoff-commit tree (`_count_repo_loc()`, new `MIN_NON_BLANK_LOC = 5000` threshold in `study_parameters.yaml`), pruning vendored/generated directories (`node_modules`, `.git`, `build`, etc. — reuses `agent_patterns.py`'s existing `_EXCLUDED_DIR_NAMES`) so a single dependency tree can't inflate the count. Runs after checkout, right before the existing test-file-count check, since both need the same checked-out tree. Stars remains the one dimension left unfixed — still no historical time series to re-measure against, so still simply dropped (see section 3).

## 11. `pushed_at` was never threaded through (found during a manual Dataset C health check, 2026-08-19)

Same class of bug as section 8's `github_id`, in the same pipeline: `repositories.pushed_at` came back as an empty string for all 1,193 repos in a fresh Dataset C collection run. Traced to a genuine implementation gap, not a regression — `pushed_at` was simply never carried through the 4-hop chain that `created_at`/`stars`/`forks`/`num_contributors` all go through, even though the raw source has it (`github-search-raw/*.csv.gz` has a real `pushedAt` column, verified directly):

1. `select_dataset_c_repos.py::select_repos()` read `createdAt` but never `pushedAt`, and `_OUTPUT_FIELDNAMES` didn't include `pushed_at` — so `datasets/c/repos/{lang}_repo.csv` never had the column to begin with.
2. `dataset_c.py::load_dataset_c_repos()` builds an explicit dict with no `pushed_at` key, so even a CSV that had the column wouldn't have been read.
3. `dataset_c.py`'s per-fixture dict (needed because `repo` itself is discarded downstream — see the `github_id` comment at that same call site) carried `repo_created_at`/`repo_stars`/etc. but no `repo_pushed_at`.
4. `dataset_c.py`'s `construct_repo_dict(...)` call passed `created_at=repo_created_at` but never `pushed_at=`, so `construct_repo_dict()`'s own `pushed_at: str = ""` default silently won every time.

No existing test caught it for the same reason section 8's bug went uncaught: nothing exercised this specific field end to end.

Fixed by adding the missing line at each of the four hops, mirroring the existing `created_at` pattern exactly (unlike `created_at`, `pushed_at` is kept at full ISO precision rather than truncated to `[:10]` — that truncation exists specifically for `created_at`'s own min/max creation-window comparison, which `pushed_at` has no equivalent of). New tests: `select_dataset_c_repos.py`'s `pushedAt`-reading (present and absent-defaults-to-`""` cases) and `load_dataset_c_repos()`'s CSV round-trip (same two cases) — matching the exact test depth already established for the sibling fields in this same file.

**Caveat, not yet acted on**: fixing the code doesn't retroactively fix repos already persisted in the in-progress collection run that surfaced this — `select_dataset_c_repos.py`'s output only gets the new column after `discover-repos --dataset c` is re-run (both passes), and `dataset_c.py`'s per-language checkpoint resume means already-`completed_repos` are never revisited even after that. Re-running `discover-repos --dataset c` before resuming the still-incomplete javascript/typescript/java/python extraction (see the same health check) picks up the fix for everything from that point forward; already-collected java/python repos keep a blank `pushed_at` unless separately backfilled or the whole dataset is re-collected from scratch.

## 12. Why the first Dataset C collection attempt didn't finish (2026-08-19)

Same health check as section 11 also surfaced this: python reached 93.6% complete, java only 37.5%, and javascript/typescript never started at all (`completed_repos_count: 0` in both progress files) — with no network or server incident during the ~40h window, and Dataset A running cleanly the whole time on the same host. Investigated by reading `dataset_c.py`'s extraction/checkpoint code directly (no server logs available). Two real, Dataset-C-specific issues found, both now fixed:

**a) Unbounded, unpruned directory walks.** `find_test_files_at_commit()` and `find_test_files_with_language()` discovered test files via `repo_path.rglob("*")` — `rglob` has no directory-pruning mechanism, so every call also walked into `.git/`, `node_modules/`, `vendor/`, `build/`, etc. in full. `_count_repo_loc()` (right next to them) already pruned those same directories via `agent_patterns.py`'s `_EXCLUDED_DIR_NAMES`, so the gap was previously known locally (its own docstring called it out) but not treated as a blocking risk. This is a structural asymmetry with Dataset A, which never walks a working tree at all — `agent_fixture_extractor.py::_find_added_test_files()` discovers test files from each commit's own bounded PyDriller diff, so its cost doesn't scale with a repo's total tree size or git history length the way Dataset C's did. For a repo with a large committed dependency tree or long git history (candidates like `eclipse/steady`, caught mid-processing in the same health check, are exactly this shape), this walk could be extremely slow — and with `workers=N` threads and no per-future timeout anywhere in `collect_dataset_c_fixtures()`, a single such repo stalls the *entire* stage: `as_completed()`'s loop can't finish until every submitted future resolves, even if every other worker sits idle waiting on the one straggler.

Fixed by extracting the pruned walk into one shared helper, `_iter_repo_files()`, and switching all three functions (`find_test_files_at_commit()`, `find_test_files_with_language()`, `_count_repo_loc()`) onto it — the same `_EXCLUDED_DIR_NAMES`-pruned `os.walk()` `_count_repo_loc()` already used, no more `rglob()` anywhere in this module.

**b) The checkpoint file wasn't updated as work actually completed.** `collect_dataset_c_fixtures()` gathers every candidate repo's fixtures first (via a `ThreadPoolExecutor`), *then* runs stratified sampling, *then* persists the selected fixtures to the DB in a final sequential loop over `repo_groups`. That final loop had no exception handling and — critically — called `_save_dataset_c_checkpoint()` exactly once, after every repo in the loop had been persisted. `persist_repository_and_fixtures()` commits each repo's rows to the DB independently as the loop runs, so a crash or interrupt partway through left real, already-committed data in `db/c.db` that the checkpoint file didn't know about — exactly the state `eclipse/steady` was found in (56 fixtures already in the DB, absent from `completed_repos`). On the next invocation, `pending_repos` is computed by excluding `completed_repos` from the full candidate list, so every one of those already-persisted repos would be silently re-cloned, re-extracted, and re-persisted (safe — `insert_fixture()`'s `ON CONFLICT ... DO NOTHING` makes re-persisting a no-op — but a large amount of wasted time on every restart, compounding across however many times the run was interrupted and restarted over the 40h window). Note this is *not* the same as the extraction-phase `except KeyboardInterrupt` handler a few lines above it in the same function, which correctly does *not* add that run's in-flight repos to the checkpoint on interrupt — at that point their fixtures haven't survived sampling or been persisted yet, so marking them complete there would have been actual data loss, not just wasted recompute.

Fixed by saving the checkpoint (and progress file) once per repo inside the final persist loop, in a `finally` block — so at most one repo's work is ever redone after an interruption, instead of the entire remaining batch. A final unconditional save after the loop is kept too, for the case where `repo_groups` ends up empty (some extraction-successful repos' fixtures don't survive sampling, so the loop body never runs for them; without that fallback save, they'd stay "pending" on disk forever despite already being done in memory).

**c) Why javascript/typescript never started at all.** No corresponding code bug found for this — the documented run sequence (`internal-docs/RUN_COMMANDS.md`) chains all four languages with `&&`, python → java → javascript → typescript. If python or java's stage was killed with a non-zero exit (rather than a clean finish), the chain would stop there and never reach the javascript/typescript commands — consistent with their progress files showing zero attempted work rather than a partial one. Most likely a consequence of (a) and (b) above (a stalled/slow stage prompting a manual interrupt) rather than a separate defect.

New tests: `test_find_test_files_with_language_skips_vendored_directories`, `test_find_test_files_at_commit_skips_vendored_directories` (vendored subtree no longer surfaces test files), `test_collect_dataset_c_checkpoint_persisted_incrementally` (checkpoint's `repos_persisted` count reflects an earlier repo in the loop before a later one is even attempted), `test_collect_dataset_c_checkpoint_keeps_earlier_repos_credit_on_later_failure` (a later repo's persistence failure doesn't roll back an earlier repo's already-saved checkpoint credit) — all in `tests/collection/test_dataset_c.py`.
